...
function usage() {
  console.error('Usage: node scripts/ldo-run.mjs --runtime codex|claude [--model MODEL] [--planner-model MODEL] [--researcher-model MODEL] [--coder-model MODEL] [--reviewer-model MODEL] [--security-model MODEL] [--research] [--plan-only] [--isolate] [--security auto|true|false] "task"')
  process.exit(2)
}

const args = process.argv.slice(2)
const options = { planOnly: false, isolate: false, research: false, security: 'auto' }
const taskParts = []
const valueFlags = new Set(['--runtime', '--model', '--planner-model', '--researcher-model', '--coder-model', '--reviewer-model', '--security-model', '--security'])
for (let index = 0; index < args.length; index++) {
  const arg = args[index]
  if (arg === '--plan-only' || arg === '--isolate' || arg === '--research') {
...
const schemas = Object.fromEntries(['researcher', 'planner', 'security', 'coder', 'reviewer'].map(role => [role, resolve(root, 'schemas', `${role}.json`)]))
...
pipeline({ task, cwd: isolation?.path || originalCwd, planOnly: options.planOnly, research: options.research, security: options.security === 'auto' ? 'auto' : options.security === 'true' }).then(result => {
